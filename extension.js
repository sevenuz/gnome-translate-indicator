...
const Translate = St.Clipboard.get_default();
...
const TranslateIndicator = Lang.Class({
    ...
    _removeAll: function () {
        let that = this;
        // We can't actually remove all items, because the translate still
        ...
        that._updateCache();
        that._showNotification(_("Translate history cleared"));
    },
    ...
    _removeEntry: function (menuItem, event) {
        ...
        if(event === 'delete' && menuItem.currentlySelected) {
            Translate.set_text(CLIPBOARD_TYPE, "");
        }
    ...
    _onMenuItemSelected: function (autoSet) {
        ...
        that.radioGroup.forEach(function (menuItem) {
            ...
            if (menuItem === that && clipContents) {
                ...
                if (autoSet !== false)
                    Translate.set_text(CLIPBOARD_TYPE, clipContents);
            }
            else {
    ...
    _refreshIndicator: function () {
        ...
        Translate.get_text(CLIPBOARD_TYPE, function (clipBoard, text) {
            that._processTranslateContent(text);
        });
    },

    _processTranslateContent (text) {
        const that = this;
        ...
        if (text !== "" && text) {
            ...
            if (itemIndex < 0) {
                ...
                if (NOTIFY_ON_COPY) {
                    that._showNotification(_("Copied to translate"), notif => {
    ...
    _setupTimeout: function (reiterate) {
        ...
        this._translateTimeoutId = Mainloop.timeout_add(TIMEOUT_MS, function () {
    ...
    _cancelNotification: function() {
        if (this.clipItemsRadioGroup.length >= 2) {
            ...
            Translate.set_text(CLIPBOARD_TYPE, previousClip.clipContents);
    ...
    _onPrivateModeSwitch: function() {
        ...
        // We hide the history in private ModeTypee because it will be out of sync (selected item will not reflect translate)
        this.scrollViewMenuSection.actor.visible = !PRIVATEMODE;
        this.scrollViewFavoritesMenuSection.actor.visible = !PRIVATEMODE;
        // If we get out of private mode then we restore the translate to old state
        if (!PRIVATEMODE) {
    ...
    _clearTranslateTimeout: function () {
        if (!this._translateTimeoutId)
            return;

        Mainloop.source_remove(this._translateTimeoutId);
